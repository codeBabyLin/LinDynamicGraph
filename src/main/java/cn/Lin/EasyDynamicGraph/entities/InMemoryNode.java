...
public class InMemoryNode implements InMemoryEntity {
    ...
    public void merge(InMemoryEntity other) {
        if (other instanceof InMemoryNode) {
            InMemoryNode otherNode = (InMemoryNode) other;
    ...
    public InMemoryNode copy() {
        InMemoryNode newNode = new InMemoryNode(this.nodeId, this.startTimestamp, this.deleted, this.diff);
        ...
        List<Pair<String,Boolean>> ls = labels.getLabels();
        for (int i = 0; i < labels.size(); i++) {
            Pair<String,Boolean> l = ls.get(i);
