...
public class LabelMap {
    ...
    private static final List<Pair<String, Boolean>> EMPTY_LIST = new ArrayList<>() ;
    ...
    private void resizeProperties() {
        if (labelSize + 1 >= labels.length) {
            int newSize = Math.max(2 * labelSize, 1);
            String[] newL = new String[newSize];
            System.arraycopy(labels, 0, newL, 0, labels.length);
            labels = newL;

            boolean[] newD = new boolean[newSize];
