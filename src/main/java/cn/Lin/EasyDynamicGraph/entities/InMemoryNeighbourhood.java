...
public class InMemoryNeighbourhood implements InMemoryEntity {
    ...
    public void merge(InMemoryEntity other) {
        ...
        if (other instanceof InMemoryNeighbourhood) {
            InMemoryNeighbourhood otherNode = (InMemoryNeighbourhood) other;
            this.startTimestamp = otherNode.startTimestamp;
            this.deleted = otherNode.deleted;
            this.relId = otherNode.relId;
        } else {
           // throw new InvalidArgumentException(String.format("Cannot merge InMemoryNeighbourhood with %s", other.toString()));
        }
    }
