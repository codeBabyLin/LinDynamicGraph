...
public class ByteEntityUtilities {
    ...
    public static void serializeEntity(
        ...
        if (entity instanceof InMemoryNode ) {
            ...
            InMemoryNeighbourhood neighbourhood = (InMemoryNeighbourhood) entity;
    ...
    public static void serializeEntity(
        ...
        List<Pair<String,Boolean>> labels = node.getLabelUpdates();
        Property[] properties = node.getPropertyUpdates();
        ...
        if (!node.isDeleted()) {
            ...
            for (Pair<String,Boolean> l : labels) {
    ...
    public static InMemoryNode deserializeNode(ByteBuffer buffer, Map<Integer, String> idsToNames) {
        ...
        InMemoryNode node = new InMemoryNode(nodeId, timestamp);
        if (isDeleted) {
            node.setDeleted();
            return node;
        }
    ...
    public static void serializeEntity(
        ...
        Property[] properties = relationship.getPropertyUpdates();
        int numOfProperties = relationship.getNumberPropertyUpdates();
        ...
        int label = relationship.getType();
        buffer.putInt(label);
    ...
    public static InMemoryRelationship deserializeRelationship(ByteBuffer buffer, Map<Integer, String> idsToNames) {
        ...
        long nodeId = buffer.getLong();
        long timestamp = buffer.getLong();
        long startNode = buffer.getLong();
        long endNode = buffer.getLong();
        int label = buffer.getInt();

        boolean isDiff = (getBit(header, 3) == 1);
        boolean isDeleted = (getBit(header, 4) == 1);

        InMemoryRelationship relationship = new InMemoryRelationship(nodeId, startNode, endNode, label, timestamp);
        if (isDeleted) {
    ...
    public static InMemoryRelationship deserializeRelationship(
        ...
        InMemoryRelationship relationship = (!withPointers)
    ...
    private static void getProperties(InMemoryEntity node, ByteBuffer buffer, Map<Integer, String> idsToNames) {
        int numberOfProperties = (int) buffer.get();
        for (int i = 0; i < numberOfProperties; ++i) {
            int propertyId = buffer.getInt();

            // Get metadata about the property type or whether it is deleted
            boolean isDeleted = (getBit(propertyId, 31) == 1);
            byte type = getByte(propertyId, 3);
            propertyId = unsetByte(propertyId, 3);

            String propertyName = idsToNames.get(propertyId);
            if (isDeleted) {
    ...
    private static Object getProperty(ByteBuffer buffer, int type) {
        Object value;
        switch (type) {
            case 0 :value= buffer.getInt();break;
            case 1 :value = buffer.getLong();break;
            case 2 :value = buffer.getFloat();break;
            case 3 :value = buffer.getDouble();break;
    ...
    private static void getLabels(InMemoryNode node, ByteBuffer buffer, Map<Integer, String> idsToNames) {
        int numberOfLabels = (int) buffer.get();
        for (int i = 0; i < numberOfLabels; ++i) {
            int labelId = buffer.getInt();
            boolean isDeleted = (getBit(labelId, 31) == 1);
            labelId = unsetBit(labelId, 31);

            String labelName = idsToNames.get(labelId);
            if (isDeleted) {
